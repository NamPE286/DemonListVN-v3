<script lang="ts">
  ...
  async function openCase() {
    ...
    const pollEntries = caseItems && caseItems.length > 0
      ? caseItems.map((c: any) => ({ item: { ...c.items, quantity: c.quantity ?? 1 }, rate: c.rate ?? 0 }))
      : [];
  ...
  onMount(() => {
    // start rolling immediately when the dialog content mounts
    // guard for provided inventoryItemId
    if (inventoryItemId) {
      // fire-and-forget
      openCase();
    }
  ...
  <div style="min-width:420px;">
    {#if isFetchingResult}
      <div class="py-6 text-center">
        <Loading inverted />
        <div class="mt-2 text-sm text-gray-300">Waiting for server response...</div>
      </div>
    {:else if isRolling}
      <div class="case-container-small">
        <div class="case-window-small">
          <div class="selector-line-small"></div>
          <div class="items-container-small" style="transform: translateX({-rollScroll}px); transition: none;">
            {#each rollDisplay as d}
              <div class="case-item-small" style="border-color: {rarityColor(d?.rarity ?? 0)}">
                {#if d && d.id && d.id !== 0}
                  <img src={`https://cdn.demonlistvn.com/items/${d.id}.webp`} alt={d.name} class="case-img" />
      ...
      <div class="p-4">
        {#if rollResult && rollResult.items}
          <div class="result-viewport">
            <div class="result-thumb" style="border-color: {rarityColor(rollResult.items.rarity ?? 0)}">
              <img src={`https://cdn.demonlistvn.com/items/${rollResult.items.id}.webp`} alt={rollResult.items.name} class="h-full w-full object-cover p-[10px]" />
            </div>
            <div class="mt-3 text-center">
              <div class="text-lg font-semibold">{rollResult.items.name} x{rollResult.quantity}</div>
