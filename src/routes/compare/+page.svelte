<script lang="ts">
	...
	$: if ($user.checked) {
		...
		// If logged in and supporter, auto-select current user as player 1 (only once)
		if (isSupporter && !player1 && $user.data && !hasAutoLoaded) {
...
<svelte:head>
	<title>Player Comparison - Demon List VN</title>
</svelte:head>

<div class="wrapper">
	{#if loading}
		<div class="flex items-center justify-center p-[100px]">
			<Loading inverted />
		</div>
	{:else if !isSupporter}
		<Card.Root class="mx-auto mt-[100px] max-w-[600px]">
			<Card.Header>
				<Card.Title class="text-center">Supporter Feature</Card.Title>
			</Card.Header>
			<Card.Content>
				<p class="text-center">
					Player comparison is an exclusive feature for supporters. Please consider supporting us to
					unlock this and other premium features!
				</p>
			</Card.Content>
			<Card.Footer class="flex justify-center gap-4">
				<Button variant="outline" on:click={() => goto('/')}>Go Home</Button>
				<Button on:click={() => goto('/supporter')}>Become a Supporter</Button>
			</Card.Footer>
		</Card.Root>
	{:else}
		<h1 class="mb-8 text-center text-4xl font-bold">Player Comparison</h1>
		...
		<div class="selection-grid">
			...
			<Card.Root class="player-select-card">
				...
				<Card.Content>
					...
					{:else}
						<Button class="w-full" on:click={() => openSelector(1)}>Select Player</Button>
					{/if}
				</Card.Content>
			</Card.Root>
		...
		{#if player1 && player2}
			<Card.Root class="comparison-card mt-8">
				<Card.Header>
					<Card.Title class="text-center">Statistics Comparison</Card.Title>
				</Card.Header>
				<Card.Content>
					<div class="comparison-table">
						...
						<div class="comparison-row">
							...
							<div class="stat-label">Platformer Rating</div>
						...
						<div class="comparison-row">
							...
							<div class="stat-label">Featured List Points</div>
						...
						<div class="comparison-row">
							...
							<div class="stat-label">Contest ELO</div>
						...
						{#if player1Records && player2Records}
							...
							<div class="comparison-row">
								...
								<div class="stat-label">Best Classic</div>
								<div
									class="stat-cell {getStatComparison(
										p2Best?.levels?.dlTop,
										p1Best?.levels?.dlTop,
