                    ...
                    {:else}
                        <div class="flex flex-col gap-4">
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                {#if isPremium}
                                    <div class="flex items-center gap-2 rounded-full px-4 py-2 text-black" style="background: linear-gradient(to right, rgba(var(--primary-color), 1), rgba(var(--primary-color), 0.8));">
                                        <Crown class="h-5 w-5" />
                                        <span class="font-bold">PREMIUM</span>
                                    </div>
                                {:else}
                                    <Button class="text-black" style="background: linear-gradient(to right, rgba(var(--primary-color), 1), rgba(var(--primary-color), 0.8));" on:click={() => (purchaseDialogOpen = true)}>
                                        <Crown class="mr-2 h-4 w-4" />
                                        {$_('battlepass.upgrade')} - {formatCurrency ? formatCurrency(PREMIUM_PRICE) : PREMIUM_PRICE}
                                    </Button>
                                {/if}
                            </div>
                            ...
                            {#if claimableRewards.length > 0}
                                <div class="flex items-center gap-2 rounded-lg bg-green-500/20 p-3 text-green-400">
                                    <Gift class="h-5 w-5" />
                                    <span class="font-medium">{$_('battlepass.rewards_ready', { values: { count: claimableRewards.length } })}</span>
                                </div>
                            {/if}
                        </div>
                    {/if}
...
<style lang="scss">
    ...
    .hero-bg-overlay {
        background: linear-gradient(135deg, rgba(var(--primary-color), 0.15) 0%, rgba(var(--primary-color), 0.1) 50%, rgba(var(--primary-color), 0.05) 100%);
    }
    ...
    @keyframes tier-glow {
        from { box-shadow: 0 0 15px rgba(var(--primary-color), 0.3); }
        to { box-shadow: 0 0 25px rgba(var(--primary-color), 0.5); }
    }

    .xp-bar-container { box-shadow: inset 0 2px 4px rgba(0,0,0,0.2); }
    .xp-bar { box-shadow: 0 0 10px rgba(var(--primary-color), 0.5); }
</style>
